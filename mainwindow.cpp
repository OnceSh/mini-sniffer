...
{
    ui->setupUi(this);
    statusBar()->showMessage("welcome to mini-sniffer!");
...
void MainWindow::showNetWorkCard(){
    ...
    if(n == -1){
        ...
        ui->comboBox->addItem("please choose a Network card!");
        for(device = all_device; device != nullptr; device = device->next){
            QString device_name = device->name;
            device_name.replace("\\Device\\", "");
            QString descrip = device->description;
            QString item = device_name + "   " + descrip;
...
int MainWindow::capture(){
    ...
    if(device){
        ...
        statusBar()->showMessage("please choose a Network card!");
        return -1;
    }
    if(!pointer){
